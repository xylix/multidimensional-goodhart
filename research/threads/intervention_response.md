...
Definition. `R` is a **pure selection channel** if `mu_theta << mu_0` for all `theta` and the policy only reweights fixed states or fixed underlying types, in which case `L_theta := d mu_theta / d mu_0` exists and `mu_theta` is the reweighting of `mu_0` by `L_theta`. An **intervention channel** changes the response/action kernel at fixed type: agents can move `(P, H)` or otherwise change the state-generating mechanism. Mutual singularity with `mu_0` is sufficient evidence of intervention, but not necessary.

Claim [tentative]: the entire iteration-1-to-3 apparatus — covariance, threshold response `b_H(t)`, weighted response `B_H(theta)` with weight `W_theta` — is the pure-selection case, with `L_theta = W_theta / E_{mu_0}[W_theta]`. Toy example: choosing which of a fixed applicant pool to admit is a selection channel; the admitted class is the pool reweighted by an admission indicator.

Counterexample attempt: the absolute-continuity part is not a causal boundary. A channel can be `<< mu_0` yet so concentrated (likelihood ratio unbounded) that it behaves like near-singular selection — e.g., selecting the single top scorer. More importantly, if `mu_0` already has `epsilon` mass on gaming-like actions, an announced metric can induce much more gaming while preserving `mu_theta << mu_0`. The relevant causal question is whether fixed types can change `(P, H)`, not just whether the induced law has a density.
...
Claim [tentative]: under a pure selection channel, hidden Goodhart drift is controlled by two things that *are* visible in the baseline distribution: how hard you reweight (`delta`) and how variable the declared hidden coordinates are (`s`). If hidden coordinates have bounded baseline variance and the policy has bounded reweighting budget, hidden drift is bounded — full stop. Toy example: promoting the top 30% by a noisy score moves any baseline-low-variance hidden trait by at most a fixed amount once the hidden coordinate system and norm are fixed; the `sqrt(d)` growth from iteration 1 is exactly the `||s||_2` term.

Counterexample attempt: the bound is vacuous when `delta` is large (extreme selection, e.g. top-`1/n`) — there `delta` can grow with sample size and the bound says little. It is also a worst-case bound, not the actual drift, which is the inner product, not the norm product. Finally, it is coordinate-explicit rather than coordinate-free: splitting one hidden variable into several correlated coordinates changes `||s||_2` unless a value-weighted covariance/operator norm is declared. So it is a sanity envelope, not a prediction. Still, the *structural* point survives: every term on the right is a `mu_0`-functional.

Claim [tentative]: no analogous baseline-only bound exists for an intervention channel. In the clean singular case `L_theta` does not exist; in the `epsilon`-baseline case it exists but describes the result of the induced response, not the cost geometry that made the response available. Any useful bound on intervention drift must instead be imported from the agent side — a feasibility set or cost function describing what re-arrangements of probability mass agents can afford. The next section computes such a bound in a toy model and shows the controlling quantity (`Delta = sqrt(2 kappa V)` below) is exogenous to `mu_0`. Toy example: how far a school accountability metric is gamed depends on how cheap it is to drill test format and how much funding rides on the score — neither is recoverable from last year's pre-policy score distribution.
...
4. **Why this is not a selection channel.** In the clean version, `mu_0` lives on `{H = 0}`; the post-intervention joint law puts positive mass on `{H > 0}` (the gamers, a set of types of positive measure map to `a > 0`). The two laws are mutually singular in the `(Q, H)`-marginal — there is no `L_theta`. If the baseline instead had a small amount of pre-existing gaming variation, singularity could disappear, but the same fixed-type action response would remain. The state was *transported* (each gamer's `(P, H)` moved along an action channel), not merely reweighted.
...
Claim [tentative]: the right top-level object is a *response channel* `R: Theta -> P(S)`. It splits into **pure selection channels** (fixed-state/type reweighting; all of iterations 1-3 live here; hidden drift `<= delta · ||s||_2` after choosing hidden coordinates and norm, every term a baseline functional) and **intervention channels** (fixed types can move in state/action space; the drift bound, if any, comes from an agent cost model — e.g. `Delta = sqrt(2 kappa V)` in the quadratic-gaming case). The selection/intervention boundary is substantive exactly when agents can move in state space rather than only choose their own inclusion. Causal Goodhart = intervention channel that breaks `P ≈ phi(G)` structurally; adversarial Goodhart = intervention channel where `pi` is chosen worst-case for the regulator. Toy example: which students to admit from a pool = selection channel; announcing the admissions formula and watching applicant behavior change = intervention channel, with gaming intensity set by test-prep economics, not by last year's score distribution.
...
- Additive metric, quadratic per-channel costs, deterministic. Solve the agent's cost-minimal allocation by Lagrange multipliers; read off `H_per(M, d)` and its dependence on `M`. Cheap, likely closed form.
...
A counterexample to "balloon-squeezing is real" would be a model where re-routing which channels are measured strictly *reduces* fixed-deficit harm — e.g. if measured channels differ in how wasteful gaming them is, steering effort onto a low-waste channel cuts `H_per`. So I expect conservation to hold only when all gaming is equally wasteful per score unit; the model should make that assumption explicit and then I should immediately relax it and watch conservation break. A counterexample to "adding a dimension can backfire" would be a conjunctive metric, where a new measured dimension is one more thing the agent is forced to satisfy — that should make per-gamer harm grow while possibly reducing entry. So the headline can't be unconditional in either direction; it has to be indexed by aggregation rule, gaming-wastefulness, and welfare object.
...
So **gaming occurs iff `K_M >= t^2 / (2V)`** (call the RHS `K_min`), and **when it occurs, fixed-deficit per-agent harm is `H_per(M, t) = sum_{j in M} a_j = lambda K_M = t` — independent of `M`.** What `M` controls is (i) *whether* `K_M` clears `K_min`, and (ii) *how* the fixed per-agent harm `t` is split: proportional to `kappa_j` across the measured channels.

Claim [tentative] (conservation under re-routing, repaired in iteration 6): with a unit-weight additive metric and equally-wasteful gaming per score unit, the regulator cannot reduce `H_per(M, d)` for a fixed pure-gaming score deficit by changing *which* channels it measures, as long as the measured set retains enough aggregate gaming capacity (`K_M >= K_min`); `H_per(M, t)` is pinned at `t` and merely re-routes. Closing a gamed channel just spreads `t` over the others. Toy example: a hospital ranked on readmission rates clamps down on coding of "readmission" — the gaming reappears as patient-selection, discharge timing, observation-status reclassification; the per-case distortion needed to clear the rank cutoff is unchanged.

Claim [tentative] (adding a gameable measured dimension backfires at population level): expanding `M` strictly increases `K_M`, which lowers the gaming cost `t^2 / (2 K_M)`, which (weakly) *enlarges* the population that finds gaming worthwhile while leaving `H_per(M, d) = d` in the unit-weight equal-harm case. With quality heterogeneity `Q ~ N(0, sigma^2)`, the population object is `H_pop(M; F_Q, V) = E[H_per(M, t-Q) 1{0 < t-Q <= sqrt(2K_M V)}]`; the gaming-eligibility cutoff is `Q >= t - sqrt(2 K_M V)`, which moves *down* as `K_M` grows — more agents game, and the fraction of selected agents who are pure gamers rises. Toy example: a university worried that "publication count" is gamed adds "grant income" and "media mentions" to the scorecard; each is independently inflatable, so the cheapest path to any target score is now cheaper, and more faculty shift from research to portfolio-padding.
...
Counterexample attempt / scope: every claim above assumes (1) unit-weight additive aggregation, (2) all gaming equally wasteful per unit score (`H_j = a_j`), (3) a fixed pure-gaming target deficit, and (4) the binding-constraint deterministic regime. Relax (2): let channel `j` also contribute `gamma_j in [0,1]` to the true goal, so `H = sum_j (1 - gamma_j) a_j`. Then `H_per(M, t) = sum_{j in M} (1 - gamma_j) t kappa_j / K_M`, which the regulator *can* shrink by steering effort onto high-`gamma` channels (measure the ones where "gaming" is half-real). So conservation is exactly the equally-wasteful-per-score idealization; in reality the regulator's job is partly to choose proxies where the cheapest way to inflate them is also partially the real thing. Relax (3) by adding quality heterogeneity and endogenous participation: `H_pop` rises with `K_M` because more agents find gaming worthwhile. Relax (4) by adding proxy noise: agents overshoot for a safety margin, so per-gamer harm rises slightly above the deterministic score deficit, but the re-routing logic remains a first-order guide. Relax (1): see below — it flips.

### Conjunctive metric flips the sign

Suppose instead `score`-passing requires `a_j >= t` for every `j in M` (the regulator demands the agent be good on *all* measured dimensions — a `min`-style or all-of-the-above rule). Then the cost-minimal way to pass is `a_j = t` for all `j in M`, cost `sum_{j in M} t^2 / (2 kappa_j)`, and per-gamer harm `t |M|`. Now harm conditional on gaming grows *linearly in the number of measured dimensions*, while population harm depends on the higher-cost entry response — a genuine tradeoff: harm-per-gamer up, gamer-count down.

Claim [tentative, repaired in iteration 6]: gaming harm's dependence on the number of measured proxy dimensions is governed by the aggregation rule, harm-per-score exchange rates, and the welfare object. Unit-weight compensatory/additive metrics conserve fixed-deficit `H_per(M, d)` only when measured channels are equally harmful per score unit; conjunctive/`min` metrics *multiply* per-gamer harm (`t|M|`) while their `H_pop` sign depends on entry. Real-world scorecards are often compensatory (weighted sums of KPIs), which is the regime where "just add another metric" can backfire for `H_pop` by cheapening the cheapest gaming path and expanding the gaming population. Toy example: a compensatory teacher-evaluation rubric (test scores + observations + surveys, weighted-summed) lets a teacher trade a bad classroom observation against great test scores, so effort flows to whichever component is cheapest to inflate; a conjunctive rubric (must clear a bar on *each*) forces gaming of every component, multiplying the distortion but also failing more teachers.
...
Claim [tentative, repaired in iteration 6]: in the intervention regime, "fight Goodhart by measuring more dimensions" has a precise predicted failure mode that depends on aggregation, exchange rates, and the welfare object. Under a compensatory/additive rule, `H_per(M, d)` is conserved only when channels are equally harmful per score unit; adding a new gameable measured channel still *lowers* the cheapest gaming cost and so recruits more gamers — an `H_pop` backfire, not a simple redistribution story. The regulator's real levers are aggregate (shrink total gaming capacity, harden channels, raise the bar relative to real signal, cut the prize) or structural (pick low-harm-per-score proxies whose cheapest inflation is partly real; change weights; or go conjunctive and accept higher per-gamer harm with potentially lower entry). This is the multidimensional, intervention-regime form of conservation Goodhart (AGENT.md open question 2), but now with an explicit exchange-rate condition. Toy example: the durable lesson from metric-gaming case studies (hospital league tables, school accountability, citation metrics, sales quotas) is that closing one gaming route often reroutes effort to the next-cheapest one; whether that keeps harm roughly constant depends on whether the substitute route is equally damaging per score point.
...
The adversarial pass must distinguish at least four objects that iteration 5 partly collapsed: (i) fixed-deficit per-agent harm `H_per(M, d)`, (ii) population harm `H_pop(M; F_Q, V)` after agents decide whether gaming is worthwhile, (iii) physical effort units `a_j`, and (iv) score units `w_j a_j` after the regulator chooses metric weights. A conservation claim is only meaningful after saying which object is conserved and under which units.
...
and its fixed-deficit per-agent harm is
...
Population counterexample. In the heterogeneous-quality extension from iteration 5, an agent with deficit `d = t - Q > 0` games iff `d <= sqrt(2 K_M V)`, and the fixed-deficit object is `H_per(M, d) = d`, not a fixed `t`. Population gaming harm is

`H_pop(M; F_Q, V) = E[H_per(M, t-Q) 1{0 < t-Q <= sqrt(2 W_M V)}]`.

In the unit-weight equal-harm case, `W_M = K_M` and `H_per(M, t-Q) = t-Q`, so this reduces to `E[(t-Q) 1{0 < t-Q <= sqrt(2K_M V)}]`. For any continuous density with positive mass in the newly admitted deficit band, this increases with `K_M`. Thus "adding a gameable channel backfires" is an `H_pop` statement: it does not say fixed-deficit `H_per` increases; it says the gaming band expands and population harm rises. Toy example: adding an easily padded "media impact" category to a faculty scorecard does not just spread the same gaming among existing borderline faculty; it makes padding worthwhile for additional below-threshold faculty.
...
Claim [tentative]: the correct additive conservation theorem is narrow but real. For a fixed score deficit `d`, quadratic separable costs, and additive score `sum w_j a_j`, `H_per(M, d)` is conserved under re-routing **iff** social harm is proportional to score contribution on every available channel (`h_j = c w_j` on the active measured set). Then `H_per(M, d) = c d`, independent of `M`; otherwise `H_per(M, d)` depends on `M` through the cost-weighted average above. Toy example: if every point of score inflation is equally socially wasteful no matter which KPI supplies it, re-routing conserves harm; if grant-padding produces less waste per score point than citation-padding, moving weight toward grant-padding can reduce harm.
