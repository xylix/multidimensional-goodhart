...
## Iteration 43 (scoped manuscript abstract and framing)

Question tackled: Should the post-Iteration-42 manuscript state be refreshed
with a compact abstract and sharper paper/abstract framing without adding new
theorem, empirical, or policy claims?

What changed: added `book/multidimensional-goodhart-abstract.typ` and its
compiled PDF; refreshed the compact paper and abstract framing around the
current response-channel geometry, selection/value drift bounds, action-cost
separation, exchange-rate condition, adaptive-hardening boundary, killed
claims, and response-modeling contract. Updated build documentation for the
abstract target.

What the edit showed: the abstract is synthesis, not new research. It keeps
the same license map as Iteration 42: deterministic adaptive hardening is
narrow and conditional; stochastic/endogenous dynamics, empirical estimates,
hidden-target credit, and broad policy claims remain outside scope.

What's open now: era 1 needs consolidation, then book/abstract spine refactor,
prose pass, and polish. The compact paper is deprecated by the current roadmap
and should be archived during the spine refactor rather than rebuilt as a
separate deliverable.

Red flags fired: manuscript freshness risk only. The repair was scoped
presentation work under `research/reviews/scoped_manuscript_review_iteration43.md`.
