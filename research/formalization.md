...
- **Additive/compensatory metric** (`score = sum_{j in M} a_j`): cost-minimal allocation is water-filling `a_j = t kappa_j / K_M` with `K_M = sum_{j in M} kappa_j`; gaming occurs iff `K_M >= t^2/(2V)`; **total harm `H = t`, independent of `M`** — closing a gamed channel only re-routes the fixed total `t` across the rest. Expanding `M` raises `K_M`, *lowers* the cheapest gaming cost `t^2/(2K_M)`, and recruits more gamers — adding a measured dimension backfires rather than redistributing-and-growing or helping.
- **Conjunctive metric** (must clear `a_j >= t` for every `j in M`): `H = t|M|` — harm scales linearly with the number of measured dimensions.

Claim [tentative]: the dimensional dependence of gaming harm is governed by aggregation, not by dimension count alone; compensatory metrics conserve total gaming harm (conservation Goodhart, open question 2), conjunctive metrics multiply it. Scope: assumes equally-wasteful gaming (`H_j = a_j`); if channel `j` also contributes `gamma_j in [0,1]` to the true goal then `H = sum_{j in M}(1-gamma_j) t kappa_j/K_M` and the regulator *can* shrink harm by steering effort onto high-`gamma` channels. Effective regulator levers under the additive rule are aggregate (shrink `K_M` below `t^2/(2V)`, harden channels, raise `t` vs. real signal, cut `V`) or structural (high-`gamma` proxies; switch to conjunctive), never re-routing among equally-hard channels.
