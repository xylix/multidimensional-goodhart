# Next steps: Iterations 29-34

This is the active roadmap after Iteration 28. It deliberately alternates
application work and review before any book promotion:

- Iteration 29: non-ML application-template hardening.
...
Build a reusable application template and fill it for the hospital
readmission / institutional scorecard case. Iteration 28 found that the
adaptive-hardening toy is useful only if future applications can declare or
estimate the frontier primitives `kappa`, `h`, `gamma`, `w`, and `V`; this
iteration tests whether those primitives can discipline a real non-ML
application.

### Work

1. Create a reusable application template in `research/applications/`,
   `research/threads/`, or `research/reviews/` with fields for:
   - type space;
   ...
   - qualitative frontier primitives `kappa`, `h`, `gamma`, `w`, and `V`;
   - evidence standard;
   - licensed calculation;
   - design or audit decision changed by the contract;
   - discriminator observation;
   - contract-failure condition.
2. Fill the template for a hospital readmission / institutional scorecard
   case, using `plans/could-do/example_draft.md` as source material.
3. Name at least one discriminator observation that would distinguish the
   chosen response contract from a plausible alternative, such as pure
   selection, harmful fixed-type intervention, harmless proxy-only
   manipulation, or real quality improvement.
4. State what would make the template wrong as a framework application, not
   merely what would revise one parameter estimate.
5. Update `verification_stack.md`, `claim_audits.md`, `open_questions.md`,
   and `iteration_log.md` only if the application sharpens the framework
   boundary.
...
signal adequacy, gaming costs, or frontier primitives.

### Scope limits

Do not start theorem work, endogenous-stakes modeling, policy-optimality
claims, or book/paper promotion. Keep the output as a research-note or
application artifact with a clear license boundary.
