...
- **Load-bearing closed results** (under stated conditions): selection δ-reweighting bound (Props 1/1'), additive conservation iff `h_j = c w_j` (Prop 4), convex score-deficit budget `m(d)` (Prop 3), deterministic finite-channel adaptive-hardening capacity (Iteration 35). The response-modeling contract is the methodological lynchpin.
...
6. **Intervention channel.** Convex score-deficit budget `m(d) = sup_λ [λd − c*(λw)]` (Prop 3). Stackelberg wedge √(2κV) as a **signature of the quadratic toy**, not a universal intervention law. Explicit refusal: this is not a welfare bound; the cost-minimal action and the worst-case-harm action are different objects.
7. **Multidimensional scorecards — the keeper.** Foreground the additive conservation iff-condition `h_j = c w_j` (Prop 4) as the project's most exportable result: a one-line generalization of Holmström-Milgrom that gives empirical designers a checkable test for whether adding a metric helped, hurt, or re-routed harm. Conjunctive aggregation flips the sign. Population entry vs per-agent re-routing as separate objects.
